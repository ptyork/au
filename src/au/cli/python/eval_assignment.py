...
from simple_git import GitRepo, Commit

from au.classroom import Assignment, Roster
from au.click import BasePath, AssignmentOptions, RosterOptions, DebugOptions
from au.tools import draw_single_line
from au.tools.datetime import get_friendly_timedelta
...
@RosterOptions(store=False).options
@click.option("-s", "--student-name", type=str,
              help="If no roster is provided, you can provide the name of the student. If neither is provided, the name will just be STUDENT_DIR.")
...
def eval_assignment(student_dir: Path,
    ...
    console = Console()
    with console.status(f"Running Git Repository checks for {student_name}", spinner="bouncingBar") as status:
        try:
            # repo = Repo()
            repo = GitRepo()
            assert(repo)
