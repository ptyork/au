...
from simple_git import get_git_repos
...
@click.command()
@click.argument("root_dir",type=BasePath(resolve_path=True), required=True)
...
def commit_all(root_dir: Path,
    ...
    console = Console()
    with console.status("Finding all local Git repositories", spinner="bouncingBall") as status:
