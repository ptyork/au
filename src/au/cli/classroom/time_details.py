...
from simple_git import GitRepo
...
from au.tools import draw_double_line
from au.tools.datetime import get_friendly_local_datetime, get_friendly_timedelta
...
def time_details(root_dir: Path,
    ...
    console = Console()
    with console.status(status="Retrieving data from GitHub Classroom", spinner="bouncingBall") as status:
    ...
    print(get_table(rows,
                    header_row=['NAME','PAST DUE','WORK TIME','COMMIT DATE','COMMIT MESSAGE'],
                    col_defs=['','','','','AT'],
